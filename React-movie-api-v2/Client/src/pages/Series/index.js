...
import { useSeriePlaying } from '../../hooks/useSeriePlaying';
import { useSerieTop } from '../../hooks/useSerieTop'
import { useSerieComing } from '../../hooks/useSerieComing'
import { useSeriePopular } from '../../hooks/useSeriePopular'
...
export default function Serie() {
    ...
    const seriePopular = useSeriePopular()
    const seriePlaying = useSeriePlaying()
    const serieComing = useSerieComing()
    const serieTop = useSerieTop()
    ...
    return <div className='h-full overflow-x-hidden bg-black flex flex-col relative'>
        ...
        <div className='w-full bg-white '>
            <Carousel className='' autoPlay={setTimeout(true, 1000)}
                ...
                {seriePopular.loading === true ?
                    ...
                    :
                    seriePopular.results.map(element => {
                        return <div className='' key={element.id}>
                            ...
                            <img className='h-96 brightness-75 blur-1' src={element.url} alt={element.name} />
                        </div>
                    })
                }
            </Carousel>
        ...
        <div className='text-black my-10 text-center w-[90%] mx-auto'>
            ...
            >
                {
                    seriePlaying.results.map(element => {
                        ...
                        key={element.id} title={element.vote_average}>
                            ...
                            <img className='h-full cursor-pointer' src={element.url} alt={element.name} 
                            id={element.id} onClick={handleDetails} />

                            <p className='text-white text-sm font-Montserrat'>{element.name}</p>
                        </div>
                    })
                }
            </ItemsCarousel>
        ...
        <div className='text-black my-10 text-center w-[90%] mx-auto'>
            ...
            >
                {
                    serieComing.results.map(element => {
                        return <div className='h-full rounded-xl flex flex-col overflow-hidden
                    justify-center border-2 border-white w-4/5 mx-auto hover:border-red-500' 
                    key={element.id} title={element.vote_average}>
